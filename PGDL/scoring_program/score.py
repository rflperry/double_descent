...
def get_eval_complexity(evals, n_samples=1):
    if METRIC == 0:
        ...
        metric = (evals * np.log(evals)).sum()
    else:
        metric = np.sum(evals ** METRIC) ** (1/METRIC)
...
if __name__ == "__main__":
    ...
    mkdir(score_dir)
    score_file = open(os.path.join(score_dir, f"{MATRIX}_l{METRIC}_scores.txt"), "w")
    html_file = open(os.path.join(score_dir, f"{MATRIX}_l{METRIC}_scores.html"), "w")
